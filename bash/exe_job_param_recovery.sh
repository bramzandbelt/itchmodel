...
do
	...
	do
		...
		qsub -I -l walltime=`echo $WALLTIME` -l mem=`echo "$MEM"mb` -v parameterization=`echo "$i_parameterization"`,n_rep=`echo "$i_n_rep"`,ouput_dir=`echo "$output_dir"` parameterization=`echo "$i_parameterization"`
	done
done
